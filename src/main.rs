...
fn main() {
    let interface_name = "en0"; // Adjust this to your network interface name
        ...
        match rx.next() {
            Ok(packet) => {
                ...
                match packet.get_ethertype() {
                    EtherTypes::Ipv4 => {
                        if let Some(header) = pnet::packet::ipv4::Ipv4Packet::new(packet.payload()) {
                            if header.get_next_level_protocol() == IpNextHeaderProtocols::Tcp {
                                if let Some(tcp) = TcpPacket::new(header.payload()) {
                                    ...
                                    if tcp.get_flags() & pnet::packet::tcp::TcpFlags::SYN != 0
                                        && tcp.get_flags() & pnet::packet::tcp::TcpFlags::ACK == 0
                                    {
                                        println!("SYN packet detected from {:?} to {:?}", tcp.get_source(), tcp.get_destination());
                                    }
                                }
                            }
                        }
